    ...
    for (int tile_start = row_start + warp_id * TILE_SIZE;
        ...
        if (nbr >= 0) {
            ...
            for (int f = lane; f < d; f += 32) {
                atomicAdd(&dV_nbr[f], P_weight * do_dst[f]);
            }
        }
        ...
        if (nbr >= 0) {
            ...
            for (int f = lane; f < d; f += 32) {
                atomicAdd(&dq_ptr[f], dS_scaled * k_dst[f]);
            }
        }
    }
...
graph_attention_backward_buckets_cuda(
    ...
    TORCH_CHECK(err == cudaSuccess, "Backward CUDA kernel failed: ", cudaGetErrorString(err));
